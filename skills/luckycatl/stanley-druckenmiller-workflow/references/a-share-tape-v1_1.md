...
## Tiering
- Core = should be in V1 daily tape
- Confirm = helpful confirmation layer
- Optional = useful but noisy / fragile
...
| STAR50 vs CSI300 | Confirm | Higher-beta innovation board confirmation | Medium | Eastmoney / Tencent / AkShare proxies | Good to have, not strictly required in V1 |
| Up/down stocks count | Core | Most direct breadth signal | Medium-High | Eastmoney / AkShare market breadth pages | Critical for “index up but tape weak” diagnosis |
| Total turnover (沪深两市成交额) | Core | Tells whether repair has real participation | High | Eastmoney / Tencent / AkShare | Pair with breadth, not alone |
| Limit-up / limit-down counts | Optional | A-share emotion extreme gauge | Medium | Eastmoney / AkShare | Useful as emotion layer only |
...
|---|---|---|---|---|---|
| Northbound net flow (single day) | Core | Best free foreign-flow proxy | High | Eastmoney / AkShare HSGT endpoints | Must use with continuity, not only one print |
| Northbound 3-day / 5-day continuity | Core | More Stan-like than one-day flow | High | Derived from same northbound source | Preferred over raw daily number |
| Northbound sector destination | Confirm | Tells whether foreign money buys banks / consumer / tech | Medium | Eastmoney / AkShare summaries | Great if available, not mandatory |
| Margin financing balance change | Confirm | Retail leverage temperature | Medium | Eastmoney / AkShare | Better as weekly / daily confirm, not thesis anchor |
| Credit-risk proxy basket (property developers / brokers / small-cap beta) | Confirm | A-share substitute for B-rated / weak-credit tape; shows whether risk capital is willing to own fragile balance sheets | Medium | Eastmoney sector quotes + selected names | Prefer basket or spread-style read over one stock |
...
| Main-force net inflow | Optional | Can help with sector heat | Medium-Low | Eastmoney / web scrape | High noise, do not make it a core anchor |
...
| Banks: joint-stock vs big state banks | Core | Asset-quality / credit-belief proxy | High | Tencent / Eastmoney quotes | Stronger than just looking at bank index |
...
| Transport / logistics / ports / highways | Confirm | China version of truck / transport tape; tells you whether goods flow and industrial demand are broadening | High | Eastmoney sector quotes / Tencent quotes | Prefer basket read over one name |
| Commercial vehicles / heavy truck proxy | Confirm | Best public proxy for “truck tape” in A-share style | Medium | Eastmoney / Tencent quotes on truck & auto parts names | Use as a demand/credit proxy, not in isolation |
...
| Optional vs staple consumer split | Confirm | Tells you if the market is paying for discretionary recovery or hiding in defensive demand | High | Eastmoney consumer sector quotes / ETF proxies | Good add-on when consumer is a core thesis |
...
| Indicator | Tier | Why it matters | Free source feasibility | Recommended sources | Notes |
|---|---|---|---|---|---|
| DR007 | Core | China equivalent of daily funding stress / ease | Medium | AkShare / interbank sources | Critical if stable source exists |
| Shibor 1W | Core | Simple local funding anchor | High | Shibor official / AkShare / web sources | Good free substitute if DR007 lags |
| China 10Y yield | Core | Local duration / macro pricing | Medium-High | ChinaBond / AkShare / public yield sources | Daily or latest usable |
| China 10Y-1Y curve slope | Core | Recovery pricing vs disbelief | Medium | Derived from China yields | Strong macro-weather anchor |
| OMO net injection | Confirm | Tells whether PBoC is actively smoothing | Medium | PBoC site / summaries | Good desk color input |
...
| HK China assets (HSCEI / FXI / KWEB) | Core | External confirmation of China-beta | High | Yahoo / Tencent / Eastmoney / Stooq | Use as confirmation, not thesis by itself |
...
| VIX / iVIX | Confirm | Global panic vs local panic split | Medium | VIX free; iVIX availability varies | Optional in V1 if local source is unstable |

---

## Recommended V1 A-share Daily Tape (Final)
...
### Optional / emotional layer
18. Limit-up / limit-down counts
19. Main-force net inflow
20. ETF subscriptions/redemptions
...
→ 北向连续性不足
→ 宽基beta得不到增量资金确认
→ 市场回到“确定性资产优先”，茅台和中概龙头比中证1000更容易先涨。
...
### If these align, downgrade toward “防御/收缩”
...
Even with this stronger A-share tape, the skill still does not replace:
