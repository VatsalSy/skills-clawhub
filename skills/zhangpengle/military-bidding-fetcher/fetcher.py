...
# 代理配置
PROXY = {
    'http': 'http://formulas:FYmbse15@62.234.10.207:7890',
    'https': 'http://formulas:FYmbse15@62.234.10.207:7890'
}
...
def fetch_weain_bidding(
    ...
    for ptype, encoded_ptype in purchase_types:
        ...
        try:
            resp = requests.get(url, timeout=10, proxies=PROXY)
            data = resp.json()
...
def fetch_military_bidding(
    ...
    for region_name, region_code in regions.items():
        ...
        while page <= max_pages:
            ...
            try:
                resp = requests.get(base_url, params=params, headers=headers, timeout=10, proxies=PROXY)
                data = resp.json()
                items = data.get('data', [])
...
def fetch_all_bidding(
    ...
    if output_path is None:
        output_path = f'/home/ubuntu/.openclaw/workspace/military-bidding/商机信息汇总_{file_date}.xlsx'
...
if __name__ == '__main__':
    ...
    df_weain, df_military = fetch_all_bidding(
        keywords=DEFAULT_CORE_KW,
        exclude_keywords=DEFAULT_EXCLUDE_KW,
        regions=DEFAULT_REGIONS,
        date=date,
